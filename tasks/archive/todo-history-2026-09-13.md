# Continuação Farol 2.0 — plano executável — 2026-09-12

## Continuação controlada — boundedness do avaliador Golden — 2026-09-13

Objetivo imediato: fechar os riscos de execução indefinida identificados na
auditoria, sem repetir gates longos enquanto o snapshot ainda muda.

- [ ] Confirmar os testes focais atuais e reproduzir qualquer falha observada.
- [ ] Cobrir RED para timeout finito, deadline global, cleanup e término das
  threads leitoras do avaliador MCP.
- [ ] Implementar somente o mínimo para que cada teste passe, preservando o
  adapter local e sem alterar contratos de integração externa.
- [ ] Rodar testes focais, Ruff, documentação, `git diff --check` e uma suíte
  proporcional.
- [ ] Fazer no máximo uma execução `core` final, monitorada e com saída nova;
  registrar o resultado sem sobrescrever evidência histórica.
- [ ] Atualizar evidências e status apenas com resultados observados; manter
  RAGFlow/OCR/book-to-skill/cutover abertos se os recursos externos estiverem
  ausentes.

### Critério de parada

Parar após a regressão proporcional e a execução core única. Não iniciar
containers, não publicar/taguear, não remover legado e não tentar novamente
um gate longo se o bloqueio externo permanecer.

Objetivo: executar somente os gates e mudanças autorizados para fechar
TK-001→TK-020 no checkout `farol-v2`, preservando o estado inicial e mantendo
abertos os tickets cuja evidência externa não exista.

## Plano

- [x] Verificar recursos externos de forma redigida: o host tem Docker/Compose,
  mas as quatro variáveis RAGFlow estão ausentes; não há runtime/fixture
  DeepDoc-OCR nem harness/entradas reais `book-to-skill`. Nenhuma infraestrutura
  foi iniciada ou parada.
- [x] Reproduzir o perfil `core` do candidato e registrar o resultado atual:
  22/22 etapas, zero falhas e zero etapas não executadas; a suíte principal e o
  clone limpo tiveram `455 passed, 10 skipped`.
- [x] Confirmar que os gates externos não podem ser executados sem os recursos
  acima; manter TK-001, TK-007, TK-011, TK-013, TK-014, TK-018 e TK-020
  abertos, sem transformar `blocked`/`not_run` em sucesso.
- [x] Manter TK-017/TK-019 sem contração: não existe `cutover_approved` atual.
- [x] Atualizar tickets/evidências/state/backlog/todo somente com comandos,
  SHAs, resultados e bloqueios observados; não publicar, taguear ou alterar
  produção.
- [ ] Fazer a revisão final do diff e registrar abaixo a evidência final,
  incluindo qualquer `blocked`/`not_run` remanescente.

## Replanejamento após a primeira reexecução — 2026-09-12

- [x] Confirmar que a falha do core é isolada em
  `tests/test_release_gates.py::test_selected_interpreter_path_preserves_virtualenv_symlink`.
- [x] Tornar o teste explicitamente condicionado à capacidade de symlink já
  reportada pelo runner; não alterar o contrato de preservação de symlink.
- [x] Reexecutar o teste focal, a suíte completa e o perfil `core`.
- [x] Registrar a contagem real de `passed`/`skipped` e manter o skip de
  infraestrutura transparente na evidência.

Motivo: a criação de symlink falhou com `WinError 1314` no host atual. Isso
impede executar esse caso de infraestrutura, mas não prova falha de
`_absolute_path_preserving_symlink`; converter a limitação em skip explícito é
consistente com os demais testes do repositório e com `_probe_symlink_capability`.

## Revisão atual — 2026-09-12

- `git status --short --branch` confirmou `farol-v2` acompanhando
  `origin/farol-v2`; `HEAD` continua em
  `93bb8894d816aad3c3b3682ccec317db1da39d45`.
- `python scripts/check_documentation.py --root . --json` retornou
  `checked_markdown=116`, `findings=[]` e `ok=true`.
- As quatro entradas necessárias ao perfil RAGFlow (`DOCOPS_RAGFLOW_ENDPOINT`,
  `DOCOPS_RAGFLOW_TOKEN`, `DOCOPS_RAGFLOW_IMAGE_DIGEST` e
  `DOCOPS_RAGFLOW_SDK_VERSION`) continuam ausentes. Uma inspeção somente leitura
  do Docker mostrou um container já em execução com `infiniflow/ragflow:v0.27.1`;
  ele não é a versão/digest exigidos e não foi iniciado, parado ou alterado.
- O preflight RAGFlow continua `status=blocked`; não houve contato com serviço,
  OCR/DeepDoc real, harness `book-to-skill`, dual-run, cutover ou contração.

## Critério de parada

Ausência de endpoint, credencial, digest, runtime autorizado ou harness mantém
o ticket correspondente aberto e impede contração/cutover; não há fallback que
converta ausência em aprovação.

# Replanejamento de boundedness após auditoria — 2026-09-13

- [ ] Escrever testes RED para rejeitar timeouts não finitos e para impedir
  que skip/blocked sejam contabilizados como gate verde.
- [ ] Implementar validação finita nos limites do adapter RAGFlow e no perfil
  externo, com erro redigido e término controlado.
- [ ] Corrigir denominadores e política de skip do runner sem transformar
  ausência de RAG em sucesso.
- [ ] Adicionar deadline global ao pipeline de gates, mantendo o timeout por
  subprocesso como limite local.
- [ ] Rodar os testes focais, a suíte proporcional e lint/diff-check.
- [ ] Atualizar evidência somente com resultados observados; manter os tickets
  externos abertos enquanto endpoint, OCR e harness real não existirem.

## Revisão de boundedness

O rerun core iniciado em 13 de setembro de 2026 foi interrompido
deliberadamente antes da etapa longa de testes, após a auditoria encontrar
ausência de deadline global. A execução não é evidência final; os artefatos
anteriores permanecem a referência do último core verde. Nenhum container foi
iniciado, parado ou alterado.

# Continuação autônoma — endurecimento e verificação — 2026-09-12

## Replanejamento boundedness — 2026-09-12

- [ ] Auditar loops de polling/paginação e subprocessos do perfil RAGFlow.
- [ ] Escrever testes focais para timeout explícito e limite de páginas.
- [ ] Implementar limites sem transformar timeout/limite em sucesso.
- [ ] Rodar testes focais, documentação, lint e `git diff --check`.
- [ ] Fazer no máximo uma execução final monitorada do gate aplicável.
- [ ] Atualizar evidências somente com resultados observados; manter bloqueios reais.

## Plano executável

- [ ] Escrever testes RED para parse não terminal, limite de `top_k` e rejeição
  de hits fora do mapping canônico.
- [ ] Corrigir o adapter RAGFlow sem alterar a autoridade da IR nem simular
  disponibilidade externa.
- [ ] Fortalecer o spike opt-in para exigir as duas páginas, locators
  verificáveis e ausência do dataset após cleanup.
- [ ] Corrigir o runner de perfil para distinguir `passed`, `blocked`,
  `not_run` e falha real, sem transformar skips em aprovação.
- [ ] Tornar a avaliação Golden explícita sobre adapter e thresholds de cutover,
  preservando o adapter local existente.
- [ ] Rodar testes focais, suíte, lint, schemas, documentação, wheel e gates
  core; executar o perfil RAGFlow somente se as entradas externas existirem.
- [ ] Atualizar tickets canônicos, evidências, `state.json`, `backlog.md` e
  `specs/farol-2/todo.md` com resultados atuais e bloqueios redigidos.
- [ ] Revisar o diff final; não emitir `cutover_approved`, não remover legado e
  não publicar, taguear ou alterar produção sem os gates exigidos.

## Critério de parada

TK-001, TK-007, TK-011, TK-013, TK-014, TK-018 e TK-020 só podem ser
verificados com evidência real dos recursos correspondentes. TK-017 e TK-019
permanecem guardados enquanto `cutover_approved` não existir.

# Troca de checkout para a branch Farol v2 — 2026-09-12

Objetivo: garantir que o checkout local esteja integralmente alinhado à
`origin/farol-v2`, substituindo o estado v1/main conforme solicitado.

## Plano executável

- [x] Confirmar `cwd`, remoto, branch, dirty state e commits locais/remotos.
- [x] Atualizar referências remotas e alinhar o checkout à `origin/farol-v2`.
- [x] Verificar branch ativa, SHA, árvore limpa e diferença em relação à `main`.
- [x] Registrar o resultado e eventuais limitações.

## Revisão

Resultado: o checkout já estava na branch `farol-v2`. O `git fetch origin
farol-v2` atualizou as referências sem novos commits e `git pull --ff-only origin
farol-v2` retornou `up-to-date`.

Verificação: `HEAD` e `origin/farol-v2` apontam para
`93bb8894d816aad3c3b3682ccec317db1da39d45`; `origin/main` aponta para
`81d5dcb2e189d00406cdd9b9e671d94e3f23cd58` e é ancestral da v2. A diferença
entre `main` e `farol-v2` é de 167 arquivos (+11.970/-36 linhas). A única
alteração local é este registro em `tasks/todo.md`; nenhum código da v2 foi
sobrescrito ou alterado localmente.
